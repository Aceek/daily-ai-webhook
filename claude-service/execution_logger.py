...
class NodeExecutionLog(BaseModel):
    ...
    name: str = Field(..., description="Name of the n8n node")
    status: str = Field(..., description="Execution status: success, error, or skipped")
    error: str | None = Field(default=None, description="Error message if node failed")


class WorkflowLog(BaseModel):
    """Complete log data for an n8n workflow execution."""

    workflow_execution_id: str = Field(..., description="Unique ID generated by n8n")
    workflow_name: str = Field(default="Unknown Workflow", description="Name of the workflow")
    started_at: datetime = Field(default_factory=datetime.now, description="Workflow start time")
    finished_at: datetime = Field(default_factory=datetime.now, description="Workflow end time")
    ...
    error_node: str | None = Field(default=None, description="Name of the node that failed")
    nodes_executed: list[NodeExecutionLog] = Field(
        default_factory=list, description="List of nodes with their execution status"
    )
    articles_count: int = Field(default=0, description="Number of articles processed")
    claude_execution_id: str | None = Field(
        default=None, description="ID of the corresponding Claude execution log"
    )
    discord_sent: bool = Field(default=False, description="Whether Discord message was sent")
...
class ExecutionLog(BaseModel):
    ...
    workflow_execution_id: str | None = Field(
        default=None, description="ID of the parent n8n workflow execution for correlation"
    )
...
class WorkflowLogger:
    """Handles creation and storage of workflow execution logs."""

    def __init__(self, logs_dir: str = "/app/logs") -> None:
        """Initialize the workflow logger with a logs directory.

        Creates the workflows subdirectory if it doesn't exist.

        Args:
            logs_dir: Path to the base logs directory.
        """
        self.logs_dir = Path(logs_dir) / "workflows"
        self.logs_dir.mkdir(parents=True, exist_ok=True)

    def _generate_filename(self, workflow_log: WorkflowLog) -> str:
        """Generate a filename based on timestamp and workflow execution ID.

        Args:
            workflow_log: The workflow log to generate filename for.

        Returns:
            Base filename without extension.
        """
        ts = workflow_log.started_at.strftime("%Y-%m-%d_%H-%M-%S")
        return f"{ts}_{workflow_log.workflow_execution_id}"

    def _format_nodes_table(self, nodes: list[NodeExecutionLog]) -> str:
        """Format nodes execution list as a Markdown table.

        Args:
            nodes: List of node execution logs.
        ...
        lines = ["| Node | Status | Error |", "|------|--------|-------|"]
        for node in nodes:
            status = status_emoji.get(node.status, node.status)
            error_short = (
                node.error[:50] + "..." if node.error and len(node.error) > 50 else (node.error or "-")
            )
            # Escape pipe characters for markdown table
            error_escaped = error_short.replace("|", "\\|")
            lines.append(f"| {node.name} | {status} | {error_escaped} |")

        return "\n".join(lines)

    def _format_markdown(self, log: WorkflowLog) -> str:
        """Format workflow log as Markdown document.

        Args:
            log: The workflow log to format.

        Returns:
            Markdown formatted string.
        """
        status_emoji = "OK" if log.status == "success" else "FAILED"
        status_text = log.status.upper()

        # Format duration
        duration_str = f"{log.duration_seconds:.2f}s"

        md = f"""# Workflow Execution Log - {log.started_at.strftime("%Y-%m-%d %H:%M:%S")}

## Metadata

| Field | Value |
|-------|-------|
| **Workflow Execution ID** | `{log.workflow_execution_id}` |
| **Workflow Name** | {log.workflow_name} |
| **Started At** | {log.started_at.isoformat()} |
| **Finished At** | {log.finished_at.isoformat()} |
| **Duration** | {duration_str} |
| **Status** | {status_emoji} {status_text} |
| **Articles Processed** | {log.articles_count} |
| **Discord Sent** | {"Yes" if log.discord_sent else "No"} |

"""

        # Add error section if workflow failed
        if log.status == "error" and log.error_message:
            error_node_info = f" (in node: `{log.error_node}`)" if log.error_node else ""
            md += f"""## Error{error_node_info}
...
"""
        ...
        # Add nodes execution table
        md += f"""## Nodes Executed

{self._format_nodes_table(log.nodes_executed)}

"""

        # Add correlation section if Claude execution ID is available
        if log.claude_execution_id:
            md += f"""## Correlation

| Related Log | ID |
|-------------|-----|
| **Claude Execution** | `{log.claude_execution_id}` |

*See logs directory for the corresponding Claude execution log.*

"""

        md += """---
*Generated by claude-service WorkflowLogger*
"""
        return md

    def save(self, workflow_log: WorkflowLog) -> tuple[Path, Path]:
        """Save workflow log as both Markdown and JSON files.
        ...
        Returns:
            Tuple of (markdown_path, json_path).
        """
        base_filename = self._generate_filename(workflow_log)

        # Save Markdown
        md_path = self.logs_dir / f"{base_filename}.md"
        md_content = self._format_markdown(workflow_log)
        md_path.write_text(md_content, encoding="utf-8")

        # Save JSON
        json_path = self.logs_dir / f"{base_filename}.json"
        json_content = workflow_log.model_dump_json(indent=2)
        json_path.write_text(json_content, encoding="utf-8")
